			...
			<form action="#" method="post" class="form label-inline uniform" name="Xu2KNB" id="Xu2KNB">
						<h3>Thông tin tài khoản</h3>
							<div class="field">
								...
								<label for="spincount">Số lần quay còn </label>
								<p class="field_info">
								<?php
									$luckywheeldb = $Database->DBFetch("SELECT * FROM `luckywheel` WHERE `account` = '".$_SESSION['username']."'");
									echo intval($luckywheeldb['free_spin_num']);
								?>
								</p>
								...
								<select id="type" class="medium" name="fieldServer" onchange="javascript:location.href = this.value;">
										<option value="?page=vongquaymayman2&server=0" selected="selected">Hãy chọn máy chủ</option>
<?php foreach ($gamesettings as $k => $v) {
	if ($_GET['server'] == $k) echo '<option value="'.$k.'" selected>'.$v['name'].'</option>';
    else echo '<option value="/?page=vongquaymayman2&server='.$k.'">'.$v['name'].'</option>';
} ?>
								</select>
							</div>
<?php if (count($luckywheel2settings[$_GET['server']]['items']) > 0) { ?>
						<h3>Quay thưởng</h3>
							...
							<script src='./js/luckywheel2.min.js?<?php echo time(); ?>'></script>
...
<?php } ?>
									<div id="wheel">
										...
										<div id="inner-wheel">
										<?php foreach($luckywheel2settings[$_GET['server']]['items'] as $index => $item) { ?>
											<div class="sec">
												<a href="/luckywheel2.php?iteminfo&id=<?php echo $index; ?>&server=<?php echo $_GET['server']; ?>">
...
<?php } ?>
							<p style="font-size: 13px; color: #cc0000;">Lưu ý:<br>- Phần thưởng sẽ được chuyển đến Thất Quốc Tổng Quản tại Chu Quốc.</p>
<?php } ?>
						</form>
